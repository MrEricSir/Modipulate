...
static void handle_error(PaError err) {
    if (err != paNoError) {
        DPRINT("An error occured while using the portaudio stream");
        DPRINT("Error number: %d", err);
        DPRINT("Error message: %s", Pa_GetErrorText( err ));
    }
}
