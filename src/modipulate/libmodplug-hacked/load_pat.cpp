...
#else
BOOL HackedCSoundFile::TestPAT(const BYTE *lpStream, DWORD dwMemLength)
#endif
// =====================================================================================
{
	PatchHeader ph;
...
// =====================================================================================
{
	...
	tt2 = (h->samples - 1) * 16 + 128;
	for( pat = 0; pat < numpat; pat++ ) {
		pattern[pat] = HackedCSoundFile::AllocatePattern(64, h->samples);
...
#else
static void PATsample(HackedCSoundFile *cs, MODINSTRUMENT *q, int smp, int gm)
#endif
{
	WaveHeader hw;
	char s[256];
...
#else
	HackedCSoundFile *of=(HackedCSoundFile *)c;
	if( !pat_numsmp() ) pat_gmtosmp(1); // make sure there is a sample
...
#else
BOOL HackedCSoundFile::ReadPAT(const BYTE *lpStream, DWORD dwMemLength)
#endif
{
